...
public class SalesFacade implements ISalesFacade {
    ...
    public List<SalesResponseModel> getAll() {
        return salesMapper.toResponseModelList(salesService.getAllSales());
    }

    @Override
    public SalesResponseModel getById(Long id) {
        return salesMapper.toResponseModel(salesService.getSaleById(id));
    }

    @Override
    public SalesResponseModel create(SalesDto dto) {
        return salesMapper.toResponseModel(salesService.createSale(dto));
    }

    @Override
    public SalesResponseModel update(Long id, SalesDto dto) {
        return salesMapper.toResponseModel(salesService.updateSale(id, dto));
    }
